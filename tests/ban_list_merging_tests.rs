...
use bllvm_node::network::ban_list_merging::{calculate_ban_list_hash, merge_ban_lists, validate_ban_entry};
...
fn test_merge_ban_lists_conflict_longer_duration() {
    ...
    let entries2 = vec![
        create_ban_entry(0x7f000001, 8333, 2000, Some("abuse".to_string())),
    ];
...
fn test_merge_ban_lists_permanent_ban_wins() {
    ...
    let entries2 = vec![
        create_ban_entry(0x7f000001, 8333, u64::MAX, Some("permanent".to_string())),
    ];
...
fn test_merge_ban_lists_merge_reasons() {
    ...
    let entries2 = vec![
        create_ban_entry(0x7f000001, 8333, 1000, Some("abuse".to_string())),
    ];
...
fn test_validate_ban_entry_future() {
    ...
    let entry = create_ban_entry(0x7f000001, 8333, future_timestamp, Some("future".to_string()));
    assert!(validate_ban_entry(&entry));
}
...
fn test_validate_ban_entry_expired() {
    let past_timestamp = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_secs() - 3600; // 1 hour in past
    
    let entry = create_ban_entry(0x7f000001, 8333, past_timestamp, Some("expired".to_string()));
...
fn test_merge_ban_lists_sorted_output() {
    let entries1 = vec![
        create_ban_entry(0x7f000002, 8333, 2000, Some("second".to_string())),
    ];
    let entries2 = vec![
        create_ban_entry(0x7f000001, 8333, 1000, Some("first".to_string())),
    ];
