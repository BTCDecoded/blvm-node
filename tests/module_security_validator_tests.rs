...
fn test_validator_allows_read_operations() {
    ...
    let payloads = vec![
        ...
        RequestPayload::GetUtxo { outpoint: bllvm_protocol::OutPoint { hash: [0u8; 32], index: 0 } },
    ];
    
    for payload in payloads {
        let result = validator.validate_request("test-module", &payload);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), bllvm_node::module::security::validator::ValidationResult::Allowed);
    }
}
