...
use blvm_node::rpc::auth::{RpcAuthManager, RpcRateLimiter, UserId, AuthToken};
...
async fn test_rate_limiter_refill_over_time() {
    ...
    let had_tokens = limiter.tokens_remaining() > 0;
    if had_tokens {
        assert!(limiter.check_and_consume());
    }
}
...
async fn test_auth_manager_ip_rate_limiting() {
    ...
    for i in 0..50 {
        let allowed1 = manager.check_rate_limit(&user1, "test_method").await;
        let allowed2 = manager.check_rate_limit(&user2, "test_method").await;
...
async fn test_auth_manager_rate_limit_recovery() {
    ...
    // Exhaust rate limit
    for _ in 0..50 {
        manager.check_rate_limit(&user).await;
    }
    
    // Should be rate limited
    assert!(!manager.check_rate_limit(&user).await);
    ...
    // May or may not be allowed depending on timing, but system should recover
