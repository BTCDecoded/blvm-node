...
fn test_validate_endpoint_multiple_allowed() {
    ...
    assert!(sandbox.validate_endpoint("https://api1.example.com").is_ok());
    assert!(sandbox.validate_endpoint("https://api2.example.com").is_ok());
    
    // Other endpoint should be denied
    assert!(sandbox.validate_endpoint("https://api3.example.com").is_err());
}
...
fn test_validate_endpoint_empty_allowed_list() {
    ...
    assert!(!sandbox.is_network_allowed());
    assert!(sandbox.validate_endpoint("https://api.example.com").is_err());
}
