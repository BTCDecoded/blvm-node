...
async fn test_delay_before() {
    ...
    let result = delay_before(Duration::from_millis(10), || async {
        42
    }).await;
...
async fn test_collect_results_empty() {
    let operations: Vec<fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<i32, String>> + Send>>> = vec![];
