...
use bllvm_node::utils::{env_or_default, env_or_else, env_opt, env_bool, env_int};
...
fn test_env_bool_true() {
    for true_value in &["true", "TRUE", "True", "1", "yes", "YES", "on", "ON"] {
        ...
        assert!(env_bool("TEST_ENV_BOOL"), "Should be true for '{}'", true_value);
    }
...
fn test_env_bool_false() {
    for false_value in &["false", "FALSE", "0", "no", "NO", "off", "OFF", "invalid"] {
        ...
        assert!(!env_bool("TEST_ENV_BOOL"), "Should be false for '{}'", false_value);
    }
    std::env::remove_var("TEST_ENV_BOOL");
}
