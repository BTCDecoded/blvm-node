...
use bllvm_node::utils::{
    log_error, log_error_async, result_to_option, err_option_to_result,
    with_default, with_default_async, with_fallback, with_fallback_async,
};
...
async fn test_log_error_async_success() {
    let result = log_error_async(|| async {
        Ok::<i32, String>(42)
...
fn test_result_to_option_ok() {
    ...
    let opt = result_to_option(result);
...
fn test_err_option_to_result_some() {
    ...
    let result: Result<i32, String> = err_option_to_result(opt, "not found");
...
fn test_with_default_some() {
    ...
    let value = with_default(opt, 100);
...
async fn test_with_default_async_some() {
    ...
    let value = with_default_async(opt, async { 100 }).await;
