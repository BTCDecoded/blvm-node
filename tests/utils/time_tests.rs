...
use bllvm_node::utils::{current_timestamp, current_timestamp_duration};
...
fn test_current_timestamp_monotonic() {
    ...
    for i in 1..timestamps.len() {
        ...
        if timestamps[i] < timestamps[i-1] {
            assert!(timestamps[i-1] - timestamps[i] < 10, "Large timestamp decrease detected");
        }
    }
}
...
fn test_current_timestamp_reasonable() {
    ...
    assert!(timestamp <= max_timestamp, "Timestamp too far in future: {}", timestamp);
}
