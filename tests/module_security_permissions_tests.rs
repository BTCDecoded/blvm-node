...
fn test_permission_checker_read_utxo_requires_permission() {
    ...
    let payload = RequestPayload::GetUtxo { outpoint: bllvm_protocol::OutPoint { hash: [0u8; 32], index: 0 } };
    assert!(checker.check_api_call("test-module", &payload).is_ok());
}
...
fn test_permission_checker_subscribe_events_requires_permission() {
    ...
    let payload = RequestPayload::SubscribeEvents { event_types: vec![] };
    assert!(checker.check_api_call("test-module", &payload).is_ok());
}

#[test]
fn test_parse_permission_string() {
    assert_eq!(parse_permission_string("read_blockchain"), Some(Permission::ReadBlockchain));
    assert_eq!(parse_permission_string("ReadBlockchain"), Some(Permission::ReadBlockchain));
    assert_eq!(parse_permission_string("read_utxo"), Some(Permission::ReadUTXO));
    assert_eq!(parse_permission_string("ReadUTXO"), Some(Permission::ReadUTXO));
