...
use bllvm_node::config::{
    ...
    ModuleConfig, ModuleResourceLimitsConfig, NetworkTimingConfig, NodeConfig,
    PruningConfig, PruningMode, RequestTimeoutConfig, RpcAuthConfig, StorageConfig,
    TransportPreferenceConfig,
};
...
fn test_pruning_config_validate_normal_mode_invalid() {
    ...
    assert!(result.unwrap_err().to_string().contains("min_recent_blocks"));
}
...
fn test_pruning_config_validate_min_blocks_to_keep_zero() {
    ...
    let result = config.validate();
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("min_blocks_to_keep"));
}
...
fn test_pruning_config_validate_auto_prune_interval_zero() {
    ...
    let result = config.validate();
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("auto_prune_interval"));
}
...
fn test_node_config_with_custom_modules() {
    ...
    assert_eq!(
        config.modules.as_ref().unwrap().enabled_modules.len(),
        2
    );
}
...
fn test_node_config_serialization_roundtrip_json() {
    ...
    assert_eq!(loaded.max_peers, config.max_peers);
    assert_eq!(loaded.enable_self_advertisement, config.enable_self_advertisement);
}
